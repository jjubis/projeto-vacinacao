...
| Dados locais | `vacinacao.db` | SQLite em WAL; ignorado pelo Git |
| Bootstrap de funcionário | `scripts/seedFuncionario.js` | Cria um funcionário padrão se não existir |

O processo inicia em `index.js`, abre `vacinacao.db`, executa a criação idempotente das tabelas/dados iniciais e monta as rotas. A interface é servida por `express.static('public')`; não há build, ORM, migrações ou framework de front-end.
...
- Um cidadão só pode se autorregistrar se CPF e nome coincidirem com um registro de `cidadaos`; cada cidadão possui no máximo um login.
